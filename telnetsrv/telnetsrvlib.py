...
# --------------------------- Output Functions -----------------------------
    ...
    def write(self, text):
        ...
        if type(text) == str:
            barray = text.encode('ascii')
        else:
            barray = text
        barray = barray.replace(IAC, IAC + IAC)
        barray = barray.replace(bytes([10]), bytes([13, 10]))
        self.writecooked(barray)

    def writecooked(self, barray):
        """Put data directly into the output queue (bypass output cooker)"""
        self.sock.sendall(barray)
    ...
    def _inputcooker_getc(self, block=True):
        ...
        self.eof = not(ret)
        self.rawq = self.rawq + ret.decode()
        if self.eof:
            raise EOFError
    ...
    def cmdHELP(self, params):
        ...
        for cmd in keys:
            ...
            if method.__doc__ == None:
                self.writeline("%s - no help for command" % cmd)
                continue
            doc = method.__doc__.split("\n")
            docp = doc[0].strip()
            docs = doc[1].strip()
